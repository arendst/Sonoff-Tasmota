/*
  xdrv_57_irmp.ino - complete integration of IRMP for Tasmota
...
enum Xdrv57_IrErrors { IE_NO_ERROR, IE_INVALID_JSON, IE_SYNTAX_IRSND, IE_UNKNOWN };

static const char Xdrv57_IrSendCommands[] PROGMEM = "|" D_CMND_IRSND ;

static void (* const Xdrv57_IrSendCommand[])(void) PROGMEM = {
  &Xdrv57_IrSendCmnd };
...
uint8_t Xdrv57_IrSendIrSendCmndJson(void)
{
  ...
  {
#if IRMP_PROTOCOL_NAMES == 1
    value = Xdrv57_ProtocolName2Id(var_proto.getStr());
#else
    return IE_SYNTAX_IRSND;
#endif
  }
  if (Xdrv57_Validate(value) == false || value == 0 || value > IRMP_N_PROTOCOLS)
    return IE_SYNTAX_IRSND;
  ...
  if (Xdrv57_Validate(value) == false || value > USHRT_MAX)
    return IE_SYNTAX_IRSND;
  ...
  if (Xdrv57_Validate(value) == false || value > UCHAR_MAX)
    return IE_SYNTAX_IRSND;
...
void Xdrv57_IrSendCmndResponse(uint32_t error)
{
  switch (error)
  {
...
void Xdrv57_IrSendCmnd(void)
{
  uint8_t error = IE_SYNTAX_IRSND;

  if (XdrvMailbox.data_len)
  {
    error = Xdrv57_IrSendIrSendCmndJson();
  }
  Xdrv57_IrSendCmndResponse(error);
}
  ...
  if (PinUsed(GPIO_IRSEND) || PinUsed(GPIO_IRRECV))
  {
    switch (function)
    {
      case FUNC_PRE_INIT:
        ...
        {
          Xdrv57_IrReceiveInit();
        }
        break;
      case FUNC_EVERY_50_MSECOND:
        if (PinUsed(GPIO_IRRECV))
        {
          Xdrv57_IrReceiveCheck();  // check if there's anything on IR side
        }
        break;
      case FUNC_COMMAND:
        if (PinUsed(GPIO_IRSEND))
        {
          result = DecodeCommand(Xdrv57_IrSendCommands, Xdrv57_IrSendCommand);
        }
        break;
    }
  }
